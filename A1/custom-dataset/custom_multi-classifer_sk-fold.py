...
def train(model, train_loader, val_loader, criterion, optimizer, num_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        # Validation
        train_accuracy = evaluate(model, val_loader)
        ...
        if val_accuracy > best_val_accuracy:
            ...
            torch.save(model.state_dict(), f'models/sk-fold/best_model{embedding_type}.pth')
...
if __name__ == "__main__":
    ...
    vocab, embedding_matrix = create_vocab_and_embeddings(train_texts, embedding_model)
    ...
    # Create datasets
    train_dataset = CustomTextDataset(train_texts, train_labels_encoded, vocab, max_length)
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(train_texts, train_labels_encoded)):
        ...
        # Train the model
        best_val_accuracy = train(model, train_loader, val_loader, criterion, optimizer, num_epochs)
        fold_results.append(best_val_accuracy)

        print(f"Best validation accuracy for fold {fold + 1}: {best_val_accuracy:.2f}%")
        print('-' * 50)

    print(f"Average validation accuracy across folds: {np.mean(fold_results):.2f}% (+/- {np.std(fold_results):.2f}%)")
    ...
    train(model, full_train_loader, val_loader, criterion, optimizer, num_epochs)

    # Load test data, best model and evaluate on test set
    ...
    model.load_state_dict(torch.load(f'models/sk-fold/best_model{embedding_type}.pth'))
